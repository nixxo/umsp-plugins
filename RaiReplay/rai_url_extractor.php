...
function rai_day($ch, $day) {
	_logDebug(">> $ch > $day");
	$items = array();
	$ff = file_get_contents("https://www.raiplay.it/guidatv/index.html?canale=$ch&giorno=$day&new");
	if (preg_match_all("@<li[\w\W]+?<\/li>@", $ff, $lis)) {
		foreach ($lis[0] as $li) {

			//if (preg_match("@<li[\w\W]+?data-ora=\"(.+?)\"[\w\W]+?data-img=\"(.+?)\"[\w\W]+?data-href=\"(.+?)\"[\w\W]+?\"info\">(.+?)<\/[\w\W]+?\"descProgram\">([\s\S]+?)<\/[\w\W]+?\/i>(.+?)<\/[\w\W]+?\"time\">(.+?)<\/[\w\W]+?<\/li>@", $li, $mm)) {
			if (preg_match("@<li[\w\W]+?data-ora=\"(.+?)\"[\w\W]+?data-img=\"(.+?)\"[\w\W]+?data-mediapolis=\"(.+?)\" [\w\W]+?\"info\">(.+?)<\/[\w\W]+?\"descProgram\">([\s\S]+?)<\/[\w\W]+?\/i>(.+?)<\/[\w\W]+?\"time\">(.+?)<\/[\w\W]+?<\/li>@", $li, $mm)) {
				/*
					1 ora
					2 img
					3 href
